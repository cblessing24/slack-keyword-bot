...
def test_keyword_mapper_can_load_keywords(session: Any, create_keyword: KeywordCreator) -> None:
    session.execute(
        "INSERT INTO keyword (channel_name, subscriber, word, active) VALUES "
        "('general', 'bob', 'hello', 1),"
        "('random', 'alice', 'tree', 1),"
        "('lunch', 'steven', 'chicken', 0)"
    )
    expected = [
        create_keyword(channel_name="general", subscriber="bob", word="hello", active=True),
        create_keyword(channel_name="random", subscriber="alice", word="tree", active=True),
        create_keyword(channel_name="lunch", subscriber="steven", word="chicken", active=False),
    ]
...
def test_keyword_mapper_can_save_keywords(session: Any, create_keyword: KeywordCreator) -> None:
    keyword = create_keyword(channel_name="general", subscriber="bob", word="hello", active=True)
    session.add(keyword)
    session.commit()
